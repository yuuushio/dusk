...
Fnt *drw_fontset_create_lts(Drw *drw, const char *lt_font[], size_t fontcount) {
  Fnt *cur, *ret = NULL;
  size_t i;
  ...
  for (i = 1; i <= fontcount; i++) {
    if ((cur = xfont_create(drw, lt_font[fontcount - i], NULL))) {
      cur->next = ret;
      ret = cur;
    }
  }
  return (drw->lt_font = ret);
}
...
Clr *drw_scm_create(Drw *drw, char *clrnames[], unsigned int alphas[],
                    size_t clrcount) {
  int hasalpha = 0;
  size_t i;
  Clr *ret;
  ...
  for (i = 0; i < clrcount; i++)
    drw_clr_create(drw, &ret[i], clrnames[i], hasalpha ? alphas[i] : 0xffU);
  return ret;
}
...
int drw_text_ltsymbol(Drw *drw, int x, int y, unsigned int w, unsigned int h,
  ...
  if (!drw || (render && !drw->scheme) || !text || !drw->lt_font)
    return 0;
  ...
  while (1) {
    ...
    while (*text) {
      ...
      for (curfont = drw->lt_font; curfont; curfont = curfont->next) {
        charexists = charexists ||
                     XftCharExists(drw->dpy, curfont->xfont, utf8codepoint);
        if (charexists) {
          XftTextExtentsUtf8(curfont->dpy, curfont->xfont, (XftChar8 *)text,
                             utf8charlen, &ext);
          ...
          if (ew + ext.xOff > w) {
            overflow = 1;
            /* called from drw_fontset_getwidth_clamp() which wants the width
             * after overflow */
            if (!render)
              ew += ext.xOff;
            else
    ...
    if (!*text || overflow) {
      ...
      if (!drw->lt_font->pattern) {
        /* Refer to the comment in xfont_create for more information. */
        die("the first font in the cache must be loaded from a font string.");
      }

      fcpattern = FcPatternDuplicate(drw->lt_font->pattern);
      ...
      if (match) {
        ...
        if (usedfont &&
          ...
          for (curfont = drw->lt_font; curfont->next; curfont = curfont->next)
            ; /* NOP */
...
int drw_text(Drw *drw, int x, int y, unsigned int w, unsigned int h,
             unsigned int lpad, const char *text, int invert, int fillbg) {
  int i, ty = 0, ellipsis_x = 0, charexists = 0, overflow = 0;
  unsigned int ew = 0, ellipsis_w = 0, ellipsis_len;
  XftDraw *d = NULL;
  Fnt *usedfont, *curfont, *nextfont;
...
void drw_arrow(Drw *drw, int x, int y, unsigned int w, unsigned int h,
               int style, Clr prev, Clr next, Clr separator) {
  if (!drw || !drw->scheme)
    return;
...
Picture drw_picture_create_resized_data(Drw *drw, char *src, unsigned int srcw,
...
Picture drw_picture_create_resized_image(Drw *drw, Imlib_Image origin,
                                         unsigned int srcw, unsigned int srch,
                                         unsigned int dstw, unsigned int dsth) {
  if (srcw <= (dstw << 1u) && srch <= (dsth << 1u))
    return drw_picture_create_centered_image(drw, origin, srcw, srch, dstw,
                                             dsth);

  return drw_picture_create_scaled_image(drw, origin, srcw, srch, dstw, dsth);
}

Picture drw_picture_create_centered_data(Drw *drw, char *src, unsigned int srcw,
                                         unsigned int srch, unsigned int dstw,
                                         unsigned int dsth) {
  Pixmap pm;
  Picture pic;
  ...
  pic = XRenderCreatePicture(
      drw->dpy, pm, XRenderFindStandardFormat(drw->dpy, PictStandardARGB32), 0,
      NULL);
...
Picture drw_picture_create_centered_image(Drw *drw, Imlib_Image origin,
                                          unsigned int srcw, unsigned int srch,
                                          unsigned int dstw,
                                          unsigned int dsth) {
  if (!origin)
  ...
  return drw_picture_create_centered_data(
      drw, (char *)imlib_image_get_data_for_reading_only(), srcw, srch, dstw,
      dsth);
}

Picture drw_picture_create_scaled_data(Drw *drw, char *src, unsigned int srcw,
                                       unsigned int srch, unsigned int dstw,
                                       unsigned int dsth) {
  Imlib_Image origin;
  Picture pic;
...
Picture drw_picture_create_scaled_image(Drw *drw, Imlib_Image origin,
                                        unsigned int srcw, unsigned int srch,
                                        unsigned int dstw, unsigned int dsth) {
  Pixmap pm;
  Picture pic;
