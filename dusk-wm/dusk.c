...
#define CLEANMASK(mask)                                                        \
  (mask & ~(numlockmask | LockMask) &                                          \
   (ShiftMask | ControlMask | Mod1Mask | Mod2Mask | Mod3Mask | Mod4Mask |      \
    Mod5Mask))
#define INTERSECT(X, Y, W, H, Z)                                               \
  (MAX(0, MIN((X) + (W), (Z)->wx + (Z)->ww) - MAX((X), (Z)->wx)) *             \
...
struct Client {
  ...
  Client *linked;
  Workspace *ws;
  Workspace *revertws; /* holds the original workspace info from when the client
                          was opened */
  Window win;
...
typedef struct {
  char *icon;
  Arg arg;
  int pos; /* indicates that the icon is to be added as a prefix (0) or suffix
              (1) */
} StackerIcon;
...
struct Workspace {
  ...
  char *iconvac; /* vacant icon (when workspace is selected, default is empty,
                    and no clients) */
...
static void clientfittomon(Client *c, Monitor *m, int *cx, int *cy, int *cw,
                           int *ch);
...
static void clientrelposmon(Client *c, Monitor *o, Monitor *n, int *cx, int *cy,
                            int *cw, int *ch);
...
static void resizeclientpad(Client *c, int x, int y, int w, int h, int xpad,
                            int ypad);
...
static int arrange_focus_on_monocle =
    1; /* used in focus to arrange monocle layouts on focus */
...
static Atom wmatom[WMLast], netatom[NetLast], allowed[NetWMActionLast],
    xatom[XLast], duskatom[DuskLast];
...
void applyrules(Client *c) {
  ...
  if (XGetWindowProperty(dpy, c->win, netatom[NetWMWindowType], 0L,
                         sizeof(Atom), False, XA_ATOM, &da, &di, &nitems, &dl,
                         &p) == Success &&
      p) {
    win_types = (Atom *)p;
  }
  ...
  /* Steam games may come through with custom class, instance and name making it
   * hard to create generic rules for them. Overriding the class with
   * "steam_app_" to make this easier. */
  if (game_id && !strstr(class, "steam_app_"))
    class = "steam_app_";

  if (enabled(Debug))
    fprintf(stderr,
            "applyrules: new client %s (%ld), class = '%s', instance = '%s', "
            "role = '%s', wintype = '%ld'\n",
  ...
  for (i = 0; i < LENGTH(clientrules); i++) {
    ...
    if ((!r->title || strstr(c->name, r->title)) &&
        ...
        (!r->wintype ||
         atomin(XInternAtom(dpy, r->wintype, False), win_types, nitems)) &&
      ...
      if (r->workspace)
        for (ws = workspaces; ws && strcmp(ws->name, r->workspace) != 0;
             ws = ws->next)
          ;
      ...
      if (enabled(Debug) || DEBUGGING(c))
        fprintf(
            stderr,
            "applyrules: client rule %d matched:\n    class: %s\n    role: "
            "%s\n    instance: %s\n    title: %s\n    wintype: %s\n    flags: "
            "%lu\n    floatpos: %s\n    workspace: %s\n    label: %s\n",
            i, r->class ? r->class : "NULL", r->role ? r->role : "NULL",
            r->instance ? r->instance : "NULL", r->title ? r->title : "NULL",
            r->wintype ? r->wintype : "NULL", r->flags,
            r->floatpos ? r->floatpos : "NULL", r->workspace,
...
/* This mimics most of what the manage function does when initially managing the
 * window. It returns 1 if rules were applied, and 0 otherwise. A window getting
 * rules applied this way will not be swallowed.
 */
int reapplyrules(Client *c) {
  Client *t;
  Window trans = None;
  ...
  if (ISCENTERED(c)) {
     ...
     */
    if (ISTRANSIENT(c) && XGetTransientForHint(dpy, c->win, &trans) &&
        (t = wintoclient(trans))) {
  ...
  /* If the client indicates that it is in fullscreen, or if the FullScreen flag
   * has been explictly set via client rules, then enable fullscreen now. */
  if (getatomprop(c, netatom[NetWMState], XA_ATOM) ==
...
int applysizehints(Client *c, int *x, int *y, int *w, int *h, int interact) {
  ...
  *h = MAX(1, *h);
  ...
  if (*w < bh)
    *w = bh;
  if (!IGNORESIZEHINTS(c) &&
      (enabled(ResizeHints) || RESPECTSIZEHINTS(c) || FREEFLOW(c))) {
...
void buttonpress(XEvent *e) {
  ...
  allow_focus = (disabled(FocusOnClick) ||
  ...
  for (i = 0; i < LENGTH(buttons); i++) {
    if (click == buttons[i].click && buttons[i].func &&
      ...
      buttons[i].func((click == ClkWorkspaceBar || click == ClkWinTitle) &&
                              buttons[i].arg.i == 0
                          ? &arg
                          : &buttons[i].arg);
    }
...
void clientfittomon(Client *c, Monitor *m, int *cx, int *cy, int *cw, int *ch) {
  if (*cx < m->wx)
    *cx = m->wx + m->gappov;
  if (*cy < m->wy)
    *cy = m->wy + m->gappoh;
...
void clientmessage(XEvent *e) {
  ...
  if (cme->window == root) {
    if (enabled(Debug)) {
      fprintf(
          stderr,
          "clientmessage: received message type of %s (%ld) for root window\n",
  ...
  if (enabled(Debug) || DEBUGGING(c)) {
    fprintf(stderr,
            "clientmessage: received message type of %s (%ld) for client %s\n",
            XGetAtomName(dpy, cme->message_type), cme->message_type, c->name);
    fprintf(stderr, "    - data 0 = %s (%ld)\n",
            (cme->data.l[0] == 0   ? "_NET_WM_STATE_REMOVE"
...
void clientmonresize(Client *c, Monitor *from, Monitor *to) {
...
void clientsmonresize(Client *clients, Monitor *from, Monitor *to) {
  if (from == to || !from || !to)
    return;
...
void clientfsrestore(Client *c) {
  if (ISTRUEFULLSCREEN(c)) {
    resizeclient(c, c->ws->mon->mx, c->ws->mon->my, c->ws->mon->mw,
                 c->ws->mon->mh);
    XRaiseWindow(dpy, c->win);
  } else if (ISFLOATING(c)) {
...
void clientsfsrestore(Client *clients) {
  Client *c;
  for (c = clients; c; c = c->next)
    clientfsrestore(c);
}

int clientscheme(Client *c, Client *s) {
  ...
  if (FREEFLOW(c))
    return sel      ? SchemeFlexSelFloat
           : active ? SchemeFlexActFloat
                    : SchemeFlexInaFloat;

  if (fwb)
    return c->arr + (sel      ? SchemeFlexSelTTB
                     : active ? SchemeFlexActTTB
                              : SchemeFlexInaTTB);
  return sel ? SchemeTitleSel : SchemeTitleNorm;
}

/* Works out a client's (c) position on a new monitor (n) relative to that of
 * the position on another (o) monitor.
 *
 * Formula:
 *                (n->ww - c->w)
 *    x = n->wx + -------------- * (c->x - o->wx)
 *                (o->ww - c->w)
 */
void clientrelposmon(Client *c, Monitor *o, Monitor *n, int *cx, int *cy,
                     int *cw, int *ch) {
...
void clienttomon(const Arg *arg) {
  Client *c = selws->sel;
  if (!c || !mons->next)
    return;
 ...
 * More specifically it moves all clients from the selected workspace on the
 * current monitor to the selected workspace on an adjacent monitor. */
void clientstomon(const Arg *arg) {
  Workspace *ws = selws;
  Monitor *m;
...
Workspace *configurenotify(XEvent *e) {
  Monitor *m;
  Bar *bar;
  Workspace *ws;
  Client *c;
...
Workspace *destroynotify(XEvent *e) {
  Client *c;
  Bar *bar;
  Window focus_return;
  ...
  if ((c = swallowingparent(ev->window))) {
    if (enabled(Debug) || DEBUGGING(c))
      fprintf(stderr,
              "destroynotify: received event for swallowing client %s\n",
              c->name);
  ...
  /* Give input focus back to the selected client when a DestroyNotify event is
   * received for an unmanaged window and the selected client does not have
   * input focus. */
 ...
 * be returned. */
Workspace *dirtows(int dir) {
  Workspace *(*next)(Monitor *, Workspace *) =
      (abs(dir) == 2 ? &nextoccmonws : &nextmonws);
  ...
  if (dir > 0) { // right circular search
    ...
    for (last = next(m, workspaces); last && (!target || last != ws);
         last = next(m, last->next))
      target = last;
  }
...
void entermon(Monitor *m, Client *next) {
...
void focus(Client *c) {
  ...
  if (c) {
    if (c->ws != selws) {
      if (c->ws == stickyws) {
        ...
        XSetWindowBorder(dpy, c->win,
                         scheme[SchemeFlexSelFloat][ColBorder].pixel);
      } else {
        if (c->ws->mon != selmon)
          selmon = c->ws->mon;
        c->ws->mon->selws = c->ws;
    ...
    if (!stickyws->sel || stickyws->sel != wintoclient(focus_return)) {
      for (bar = selmon->bar; bar && !bar->showbar; bar = bar->next)
        ;
      XSetInputFocus(dpy, bar ? bar->win : root, RevertToPointerRoot,
                     CurrentTime);
      XDeleteProperty(dpy, root, netatom[NetActiveWindow]);
      ws->sel = ws->stack;
    }
  ...
  if (arrange_focus_on_monocle && ws->layout->arrange == flextile &&
      (ws->ltaxis[MASTER] == MONOCLE || ws->ltaxis[STACK] == MONOCLE ||
...
void focusstack(const Arg *arg) {
  ...
  if (arg->i > 0) {
    for (; ws && !c; ws = (ws->next ? ws->next : workspaces)) {
      if (!ws->visible ||
          (enabled(RestrictFocusstackToMonitor) && ws->mon != selws->mon))
        continue;
      for (c = (firstws ? selws->sel : ws->clients);
           c &&
           (c == selws->sel || ISINVISIBLE(c) || (arg->i == 1 && HIDDEN(c)));
           c = c->next)
        ;
      firstws = 0;
      if (n++ > LENGTH(wsrules))
    ...
    do {
      ...
      if (!c) {
        ...
        for (w = workspaces; w; w = w->next) {
          if (w->visible &&
              (disabled(RestrictFocusstackToMonitor) || w->mon == prevws->mon))
            ws = w;
          if (ws && w->next == prevws)
            break;
        }
...
void grabbuttons(Client *c, int focused) {
  ...
  for (i = 0; i < LENGTH(buttons); i++) {
    ...
    if ((disabled(AllowNoModifierButtons) || ONLYMODBUTTONS(c)) &&
        buttons[i].mask == 0)
      continue;
...
void grabkeys(void) {
  ...
  for (i = 0; i < LENGTH(keys); i++)
    for (j = 0; j < LENGTH(modifiers); j++)
      XGrabKey(dpy, keys[i].keycode, keys[i].mod | modifiers[j], root, True,
...
void hide(Client *c) { XMoveWindow(dpy, c->win, c->x, HEIGHT(c) * -2); }
...
int isatomstate(XClientMessageEvent *cme, int atom) {
...
void manage(Window w, XWindowAttributes *wa) {
  ...
  c->y = MAX(c->y,
             ((m->bar && m->bar->by == m->my) && (c->x + (c->w / 2) >= m->wx) &&
              (c->x + (c->w / 2) < m->wx + m->ww))
                 ? bh
                 : m->my);
  ...
  /* If this is a transient window for a window that is managed by the window
   * manager, then it should be floating. */
  if (t)
    c->prevflags |= Floating;
  if (ISTILED(c) && (ISFIXED(c) || WASFLOATING(c) ||
                     getatomprop(c, duskatom[IsFloating], AnyPropertyType)))
    SETFLOATING(c);
  ...
  if (!ISTRUEFULLSCREEN(c) &&
      (ISCENTERED(c) || (c->x == m->mx && c->y == m->my))) {
    /* Transient windows are centered within the geometry of the parent window
     */
    if (t) {
      c->sfx = c->x = t->x + WIDTH(t) / 2 - WIDTH(c) / 2;
      c->sfy = c->y = t->y + HEIGHT(t) / 2 - HEIGHT(c) / 2;
      /* Non-swallowed windows raised via terminal are centered within the
       * geometry of terminal if it fits */
  ...
  if (!c->swallowing) {
    if (riopid && (riopid == 1 || RIODRAWNOMATCHPID(c) ||
                   isdescprocess(riopid, c->pid))) {
      riopid = 0;
...
void motionnotify(XEvent *e) {
  ...
  // if (enabled(Debug)) {
  // 	if (ev->window == root) {
  // 		fprintf(stderr, "motionnotify: received event x = %d, y = %d for
  // root window\n", ev->x_root, ev->y_root); 	} else if (bar) {
  // fprintf(stderr, "motionnotify: received event x = %d, y = %d for bar %s\n",
  // ev->x_root, ev->y_root, bar->name); 	} else if ((sel =
  // wintoclient(ev->window))) { 		fprintf(stderr, "motionnotify:
  // received event x = %d, y = %d for client %s\n", ev->x_root, ev->y_root,
  // sel->name); } else { 		fprintf(stderr, "motionnotify: received
  // event x = %d, y = %d for no window?\n", ev->x_root, ev->y_root);
  // 	}
  // }
  ...
  if (!ISSTICKY(selws->sel) && (ws = recttows(ev->x_root, ev->y_root, 1, 1)) &&
      ws != selws) {
    if (selmon != ws->mon) {
  ...
  if (!ISSTICKY(selws->sel) &&
      (m = recttomon(ev->x_root, ev->y_root, 1, 1)) != selmon) {
    entermon(m, NULL);
    focus(NULL);
  }
}
...
void propertynotify(XEvent *e) {
  ...
  /* Some programs may end up spamming property notifications rendering the
   * window manager unable to do anything else than to process the backlog of
   * these for as long as it takes. One may think of it as a DOS attack for the
   * window manager. This section handles throttling of such events allowing
   * only three consecutive and identical property notifications to be
   * processed. */
  if (ev->state != pn_prev_state || ev->window != pn_prev_win ||
      ev->atom != pn_prev_atom) {
    ...
    if (pn_prev_count == 4 && enabled(Debug)) {
      ...
      fprintf(stderr,
              "propertynotify: throttling repeating %s (%ld) property "
              "notificatons for window %ld\n",
  ...
  if (ev->state == PropertyDelete) {
    if (enabled(Debug)) {
      if ((c = wintoclient(ev->window))) {
        fprintf(stderr,
                "propertynotify: ignored property delete event %s (%ld) for "
                "client %s\n",
    ...
    if ((enabled(Debug) || DEBUGGING(c)) && ev->atom != netatom[NetWMUserTime])
      fprintf(
          stderr,
          "propertynotify: received message type of %s (%ld) for client %s\n",
          XGetAtomName(dpy, ev->atom), ev->atom, c->name);
...
void raiseclient(Client *c) {
  ...
  /* If the raised client is on the sticky workspace, then refer to the
   * previously selected workspace when for searching other clients. */
 ...
 * index (idx) value accordingly. This information can later be used to
 * determine whether one window is on top of another, for example in
 * recttoclient.
 */
void readclientstackingorder(void) {
  unsigned int i, num;
...
Client *recttoclient(int x, int y, int w, int h, int include_floating) {
  Client *c, *r = NULL;
  int a, area = 1;
...
Workspace *recttows(int x, int y, int w, int h) {
  Workspace *ws, *r = NULL;
  int a, area = 0;

  for (ws = workspaces; ws; ws = ws->next)
    if (ws->visible && ws != stickyws &&
        (a = INTERSECT(x, y, w, h, ws)) > area) {
      area = a;
      r = ws;
    }
  return r;
...
void resize(Client *c, int tx, int ty, int tw, int th, int interact) {
  int wh = tw, hh = th;
  if (ISLOCKED(c)) {
    c->x = tx;
    c->y = ty;
...
void resizeclientpad(Client *c, int x, int y, int w, int h, int tw, int th) {
  XWindowChanges wc;
...
void restack(Workspace *ws) {
  ...
  raised =
      (enabled(FocusedOnTopTiled) || FREEFLOW(c) || ISTRUEFULLSCREEN(c) ? c
                                                                        : NULL);
...
void setbackground(void) {
  ...
  if (XGetWindowProperty(dpy, root, XInternAtom(dpy, "_XROOTPMAP_ID", False),
                         0L, sizeof atom, False, AnyPropertyType, &da, &di, &dl,
                         &dl, &p) == Success &&
      p) {
    return;
  }
...
void setfullscreen(Client *c, int fullscreen, int restorefakefullscreen) {
  Monitor *m = c->ws->mon;
  int savestate = 0, restorestate = 0;
  int x, y, w, h;

  if ((!ISFAKEFULLSCREEN(c) && fullscreen &&
       !ISFULLSCREEN(c)) // normal fullscreen
      || (RESTOREFAKEFULLSCREEN(c) &&
    ...
    savestate = 1;     // go actual fullscreen
           ...
           || ((RESTOREFAKEFULLSCREEN(c) || restorefakefullscreen) &&
               !fullscreen)) // fullscreen exit --> fake fullscreen
    restorestate = 1;        // go back into tiled

  /* If leaving fullscreen and the window was previously fake fullscreen, then
   * restore that while staying in fullscreen. The exception to this is if we
   * are in said state, but the client itself disables fullscreen
   * (restorefakefullscreen) then we let the client go out of while keeping fake
   * fullscreen enabled (as otherwise there will be a mismatch between the
   * client and the window manager's perception of the client's fullscreen
   * state). */
   ...
   * add an additional bit-lock so that settings can only be stored and restored
...
void setup(void) {
  ...
  scheme[LENGTH(colors)] =
      drw_scm_create(drw, colors[0], default_alphas,
                     3); // ad-hoc color scheme used by status2d
  ...
  if (!drw_fontset_create_lts(drw, lt_font, LENGTH(lt_font))) {
    die("no fonts could be loaded.");
  }
  ...
  /* One off calculating workspace label widths, used by WorkspaceLabels
   * functionality */
  occupied_workspace_label_format_length =
      TEXT2DW(occupied_workspace_label_format) -
      TEXTW(workspace_label_placeholder) * 2;
  vacant_workspace_label_format_length =
      TEXT2DW(vacant_workspace_label_format) -
      TEXTW(workspace_label_placeholder);
...
void show(Client *c) { XMoveWindow(dpy, c->win, c->x, c->y); }

void skipfocusevents(void) {
  XEvent ev;
  while (XCheckMaskEvent(dpy, EnterWindowMask, &ev))
    ; // skip any new EnterNotify events
}

void spawn(const Arg *arg) { spawncmd(arg, 0, 1); }

pid_t spawncmd(const Arg *arg, int buttonclick, int orphan) {
  struct sigaction sa;
  pid_t pid = fork();
  int i;
...
void structurenotify(XEvent *e) {
  ...
  do {
    ...
    prevws = ws;
      ...
      XCheckMaskEvent(dpy, StructureNotifyMask | SubstructureNotifyMask, e));
...
void unfocus(Client *c, int setfocus, Client *nextfocus) {
  ...
  if (!c)
    return;
  if (ISTRUEFULLSCREEN(c) && ISVISIBLE(c) && c->ws == selws && nextfocus &&
      ISTILED(nextfocus) && !STEAMGAME(c)) {
    setfullscreen(c, 0, 0);
  }
  grabbuttons(c, 0);
  if (setfocus) {
  ...
  if (enabled(FlexWinBorders))
    ...
    XSetWindowBorder(dpy, c->win,
                     scheme[clientscheme(c, nextfocus)][ColBorder].pixel);
...
void unmanage(Client *c, int destroyed) {
  ...
  if (c->swallowing) {
    unswallow(&((Arg){.v = c}));
  } else if (enabled(AutoReduceNmaster) && ws->nmaster > 1 &&
             ismasterclient(c)) {
    ws->nmaster--;
  }
 ...
 *     Bool from_configure;  // true if the event was generated as a result of a
 * resizing of the
 *                           // window's parent when the window itself had a
 * win_gravity of
 *                           // UnmapGravity
 * } XUnmapEvent;
 *
 ...
 */
Workspace *unmapnotify(XEvent *e) {
  Client *c;
  XUnmapEvent *ev;
  Workspace *ws = NULL;
  ...
  if (enabled(Debug))
    fprintf(stderr,
            "unmapnotify: received event type %s (%d), serial %ld, window %ld, "
            "event %ld, ev->send_event = %d, ev->from_configure = %d\n",
            XGetAtomName(dpy, ev->type), ev->type, ev->serial, ev->window,
            ev->event, ev->send_event, ev->from_configure);
  ...
  if ((c = wintoclient(ev->window))) {
    ...
    if (enabled(Debug) || DEBUGGING(c))
      fprintf(stderr, "unmapnotify: window %ld --> client %s (%s)\n",
              ev->window, c->name,
              ev->send_event ? "WithdrawnState" : "unmanage");
    if (ev->send_event)
      setclientstate(c, WithdrawnState);
    else
      unmanage(c, 0);
...
int updategeom(int width, int height) {
  int dirty = 0;
  ...
  if (XineramaIsActive(dpy)) {
    ...
    for (m = mons; m && m->num < nn; m = m->next) {
      if (m->num >= n || unique[m->num].x_org != m->mx ||
          unique[m->num].y_org != m->my || unique[m->num].width != m->mw ||
          unique[m->num].height != m->mh) {
        dirty = 1;
...
int xerror(Display *dpy, XErrorEvent *ee) {
  ...
  if (ee->error_code == BadWindow) {
    ...
    if (enabled(Debug))
      fprintf(stderr,
              "xerror: got XErrorEvent type %d serial %ld, error code %d "
              "request code %d minor code %d resource ID %ld\n",
              ee->type, ee->serial, ee->error_code, ee->request_code,
              ee->minor_code, ee->resourceid);
    return 0;
  }
