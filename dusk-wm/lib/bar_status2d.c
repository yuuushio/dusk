...
int size_status(Bar *bar, BarArg *a) {
  return (bar->vert ? bh : status2dtextlength(rawstatustext[a->value]));
}

int click_status(Bar *bar, Arg *arg, BarArg *a) {
  statusclicked = a->value;
  return ClkStatusText;
}

int draw_status(Bar *bar, BarArg *a) {
  return drw_2dtext(drw, a->x, a->y, a->w, a->h, a->lpad,
                    rawstatustext[a->value], 0, 1, a->scheme);
}

int drw_2dtext_ltsymbol(Drw *drw, int x, int y, unsigned int w, unsigned int h,
                        unsigned int lpad, char *text2d, int invert, int drawbg,
                        int defscheme) {
  if (!w && drawbg)
    return 0;
  ...
  while (text[++i]) {
    if (text[i] == '^' && !isCode) {
      ...
      if (tw) {
        drw_text_ltsymbol(drw, dx, y, tw, bh, 0, text, invert, fillbg);
        dx += tw;
        mw -= tw;
      }
      ...
      while (text[++i] != '^') {
        if (text[i] == 'c') {
          ...
          drw_clr_create(drw, &drw->scheme[ColFg], buf,
                         enabled(Status2DNoAlpha) ? 0xff
                                                  : default_alphas[ColFg]);
          i += 7;
        } else if (text[i] == 'b') {
          char buf[8];
          if (i + 7 > len) {
           ...
           * filesystems and a maxixmum path of 4096 characters. For status
           * updates we do not expect that long path names so we keep this at
           * 255 characters. */
          int maxlen = 256;
          char buf[maxlen];
          int use_cache = 1;
          ...
          if (text[i] == 'I') {
            use_cache = 0;
            *(text2d + i) = 'i'; // ensure that the next time this status is
                                 // used we do it from cache
          }

          for (j = 0, i++; j < maxlen - 1 && i < len && text[i] != '^';
               i++, j++)
            buf[j] = text[i];
          buf[j] = '\0';
          i--;
          ...
          if ((image = loadimage(buf, use_cache))) {
            drw_pic(drw, dx, y + (h - image->ich) / 2, MIN(image->icw, mw),
                    image->ich, image->icon);
            dx += image->icw;
            mw -= image->icw;
          }
        } else if (text[i] == 'f') {
...
int drw_2dtext(Drw *drw, int x, int y, unsigned int w, unsigned int h,
               unsigned int lpad, char *text2d, int invert, int drawbg,
               int defscheme) {
  if (!w && drawbg)
    return 0;
...
void setstatus(const Arg args[], int num_args) {
  const BarRule *br;
...
int status2dtextlength(char *text2d) {
  int i, j, w, len;
  short isCode = 0;
  char *text = {0};
  char *p = {0};
...
void statusclick(const Arg *arg) {
  spawncmd(&((Arg){.v = statusclickcmd}), arg->i, 1);
}

Image *loadimage(char *path, int use_cache) {
  int i;
  int least = -1;
  ...
  /* If we did not find our image in the buffer then we need to try to load it
   * from file. We may be overwriting the oldest image in the buffer, so free
...
int loadimagefromfile(Image *image, char *path) {
  Imlib_Image im;
  int w, h, s, ich, icw;
...
void cleanup2dimagebuffer(void) {
  int i;
