...
int draw_flexwintitle(Bar *bar, BarArg *a) {
  if (!bar->mon->selws)
    return 0;
  return flextitlecalculate(bar, a->x + a->lpad, a->w - a->lpad - a->rpad, -1,
                            flextitledraw, NULL, a);
}
...
void getclientcounts(Workspace *ws, int *n, int *clientsnmaster,
                     int *clientsnstack, int *clientsnstack2,
                     int *clientsnfloating, int *clientsnhidden) {
  Client *c;
...
int isdualstacklayout(Workspace *ws) {
  ...
  return (layout == SPLIT_HORIZONTAL_DUAL_STACK ||
...
int iscenteredlayout(Workspace *ws, int n) {
  ...
  return ((layout == SPLIT_CENTERED_VERTICAL && (n - ws->nmaster > 1)) ||
...
void flextitledrawarea(Workspace *ws, Client *c, int x, int w, int num_clients,
                       int titlescheme, int draw_tiled, int draw_hidden,
                       int draw_floating, int passx,
                       void (*tabfn)(Workspace *, Client *, int, int, int, int,
                                     Arg *arg, BarArg *barg),
                       Arg *arg, BarArg *barg) {
  int i, rw, cw, padw;
  ...
  for (i = 0; c && i < num_clients; c = c->next) {
    if (!SKIPTASKBAR(c) && !ISINVISIBLE(c) &&
      ...
      tabfn(ws, c, passx, x + padw, cw + (i < rw ? 1 : 0), titlescheme, arg,
            barg);

      if (drawpowerline && tabfn == flextitledraw && !firstpwlwintitle &&
          prevscheme > -1 && barg->lastscheme > -1)
        drw_arrow(drw, x, barg->y, sepw, barg->h, barg->value,
                  scheme[prevscheme][ColBg], scheme[barg->lastscheme][ColBg],
...
void flextitledraw(Workspace *ws, Client *c, int unused, int x, int w,
                   int tabscheme, Arg *arg, BarArg *barg) {
  if (!c)
    return;
  ...
  int i;
  char *name =
      (enabled(AltWindowTitles) && c->altname[0] ? c->altname : c->name);
  ...
  /* This works out the colour scheme to use and fills the background with a
   * solid block */
  ...
  /* This works out how much padding to add on the left side before drawing the
   * window title */
  if (w <
      textw_single_char + lrpad) { // reduce text padding if title is too small
    lpad = MAX(0, (w - textw_single_char) / 2);
    tx += lpad;
    tw -= lpad;
  } else if (enabled(CenteredWindowName) &&
             titlewidth + icon2dwidth + lrpad + ipad < w) {
  ...
  for (i = 0; i < max_items && order[i]; i++) {
    ...
    switch (order[i]) {
    case window_icon:
      drw_pic(drw, tx, barg->y + (barg->h - c->ich) / 2, c->icw, c->ich,
              c->icon);
      tx += c->icw;
      tw -= c->icw;
      break;
    ...
    case stacker_icon:
      ...
      drw_2dtext(drw, tx, barg->y, tw, barg->h, 0, stackericon->icon, 0, 1,
                 barg->lastscheme);
      tx += icon2dwidth;
      tw -= icon2dwidth;
      break;
...
void flextitleclick(Workspace *ws, Client *c, int passx, int x, int w,
                    int unused, Arg *arg, BarArg *barg) {
  if (passx >= x && passx <= x + w)
    arg->v = c;
}

int flextitlecalculate(Bar *bar, int offx, int tabw, int passx,
                       void (*tabfn)(Workspace *, Client *, int, int, int, int,
                                     Arg *arg, BarArg *barg),
                       Arg *arg, BarArg *a) {
  ...
  int clientsnmaster = 0, clientsnstack = 0, clientsnstack2 = 0,
      clientsnfloating = 0, clientsnhidden = 0;
  int i, w, r, x, den;
  int rw, rr;
  ...
  /* This avoids drawing a separator on the left hand side of the wintitle
   * section if there is a border and the wintitle module rests at the left
   * border. */
  ...
  /* This avoids drawing a separator on the right hand side of the wintitle
   * section if there is a border and the wintitle module rests at the right
   * border. */
  if (!drawpowerline && a->x + a->w < bar->bw - bar->borderpx)
    tabw -= flexwintitle_separator;

  getclientcounts(ws, &n, &clientsnmaster, &clientsnstack, &clientsnstack2,
                  &clientsnfloating, &clientsnhidden);
  ...
  XFillRectangle(drw->dpy, drw->drawable, drw->gc, a->x + a->lpad, a->y,
                 a->w - a->lpad - a->rpad, a->h);

  /* floating mode */
  if (!ws->layout->arrange) {
    flextitledrawarea(ws, ws->clients, offx, tabw, n, 0,
                      flexwintitle_masterweight, flexwintitle_hiddenweight,
                      flexwintitle_floatweight, passx, tabfn, arg, a);
    return 1;
  }
  ...
  if (clientsnmaster && flexwintitle_masterweight)
    mas_w =
        clientsnmaster * rw + w * clientsnmaster * flexwintitle_masterweight +
        (rr > 0 ? MIN(rr, clientsnmaster) : 0) + (clientsnmaster - 1) * sepw;
  rr -= clientsnmaster;
  if (clientsnstack && flexwintitle_stackweight)
    st1_w = clientsnstack * (rw + w * flexwintitle_stackweight) +
            (rr > 0 ? MIN(rr, clientsnstack) : 0) + (clientsnstack - 1) * sepw;
  rr -= clientsnstack;
  if (clientsnstack2 && flexwintitle_stackweight)
    st2_w = clientsnstack2 * (rw + w * flexwintitle_stackweight) +
            (rr > 0 ? MIN(rr, clientsnstack2) : 0) +
            (clientsnstack2 - 1) * sepw;
  rr -= clientsnstack2;
  if (clientsnhidden && flexwintitle_hiddenweight)
    hid_w = clientsnhidden * (rw + w * flexwintitle_hiddenweight) +
            (rr > 0 ? MIN(rr, clientsnhidden) : 0) +
            (clientsnhidden - 1) * sepw;
  rr -= clientsnhidden;
  if (clientsnfloating && flexwintitle_floatweight)
    flt_w = clientsnfloating * (rw + w * flexwintitle_floatweight) +
            (rr > 0 ? MIN(rr, clientsnfloating) : 0) +
            (clientsnfloating - 1) * sepw;
  if (rr > 0)
    mas_w += rr;
  ...
  for (i = 0, x = offx; i < LENGTH(order); i++) {
    ...
    case 1:
      if (clientsnmaster) {
        w = mas_w;
        flextitledrawarea(ws, ws->clients, x, w, clientsnmaster, 0, 1, 0, 0,
                          passx, tabfn, arg, a); // master
      }
      break;
    case 2:
      if (clientsnstack) {
        w = st1_w;
        flextitledrawarea(ws, nthtiled(ws->clients, clientsnmaster + 1, 0), x,
                          w, clientsnstack, 0, 1, 0, 0, passx, tabfn, arg,
                          a); // stack1
      }
      break;
    case 3:
      if (clientsnstack2) {
        w = st2_w;
        flextitledrawarea(
            ws, nthtiled(ws->clients, clientsnmaster + clientsnstack + 1, 0), x,
            w, clientsnstack2, 0, 1, 0, 0, passx, tabfn, arg, a); // stack2
      }
      break;
    case 4:
      if (clientsnhidden) {
        w = hid_w;
        flextitledrawarea(ws, ws->clients, x, w, clientsnhidden, 0, 0, 1, 0,
                          passx, tabfn, arg, a); // hidden
      }
      break;
    case 5:
      if (clientsnfloating) {
        w = flt_w;
        flextitledrawarea(ws, ws->clients, x, w, clientsnfloating, 0, 0, 0, 1,
                          passx, tabfn, arg, a); // floating
      }
      break;
    }
    x += w;
