/* Compile-time check to make sure that the number of bar rules do not exceed
 * the limit */
struct NumBarRules {
  char TooManyBarRules__Increase_BARRULES_macro_to_fix_this
      [LENGTH(barrules) > BARRULES ? -1 : 1];
};

void barhover(XEvent *e, Bar *bar) {
  if (!bar)
    return;
  ...
  for (r = 0; r < LENGTH(barrules); r++) {
    ...
    if (br->monitor != 'A' && br->monitor != -1 && br->monitor != bar->mon->num)
      continue;
    if (bar->vert && (bar->p[r] > ev->y ||
                      ev->y > bar->p[r] + br->lpad + bar->s[r] + br->rpad))
      continue;
    if (!bar->vert && (bar->p[r] > ev->x ||
                       ev->x > bar->p[r] + br->lpad + bar->s[r] + br->rpad))
      continue;
...
void barpress(XButtonPressedEvent *ev, Monitor *m, Arg *arg, int *click) {
  Bar *bar;
  const BarRule *br;
  BarArg barg = {0, 0, 0, 0};
  int barclick, r;
...
void createbar(const BarDef *def, Monitor *m) {
  Bar *bar;
  ...
  bar->borderpx = enabled(BarBorder) ? barborderpx : 0;
  m->bar = bar;
}
...
void drawbarwin(Bar *bar) {
  ...
  for (r = 0; r < LENGTH(barrules); r++) {
    br = &barrules[r];
    bar->s[r] = 0;
    if (br->bar != bar->idx || !br->sizefunc ||
        (br->monitor == 'A' && bar->mon != selmon))
      continue;
    if (br->monitor != 'A' && br->monitor != -1 &&
        br->monitor != bar->mon->num &&
        !(br->drawfunc == draw_systray && br->monitor > lastmon->num &&
          bar->mon->num == 0)) // hack: draw systray on first monitor if the
                               // designated one is not available
      continue;
    ...
    if (br->drawfunc == draw_powerline) {
      ...
      /* If the powerline is at the start or end of the bar, then keep the
       * powerline but reduce the size by half. When drawn this will be made a
       * solid block rather than slashes or arrows. */
    ...
    if (br->drawfunc && br->drawfunc != draw_powerline) {
      ...
      bar->sscheme[r] =
          (barg.firstscheme != -1 ? barg.firstscheme : barg.scheme);
...
void drawbarmodule(const BarRule *br, int r) {
  Monitor *m;
  Bar *bar;
  BarArg barg = {0};
  barg.lpad = br->lpad;
...
void setbarpos(Bar *bar) {
  ...
  switch (sscanf(bar->barpos, "%f%c %f%c %f%c %f%c", &x, &xCh, &y, &yCh, &w,
                 &wCh, &h, &hCh)) {
  case 8:
    // all good
    break;
  default:
    fprintf(stderr,
            "Bar %s (%d) on monitor %d, bad barpos '%s' - can't place bar\n",
...
void reducewindowarea(Monitor *m) {
  Bar *bar;
...
void togglebarpadding(const Arg *arg) {
  Bar *bar;
...
void togglecompact(const Arg *arg) {
  Bar *bar;
  Monitor *m;
  Workspace *ws;
